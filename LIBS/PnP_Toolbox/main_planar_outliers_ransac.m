...
f = {@kP3P,@RPnP,@kP3P,@kP3P, @REPPnP_planar};
...
for i= 1:length(pouts)
        ...
        for j= 1:num
            ...
            for k= 1:length(method_list)
                 if strcmp(method_list(k).name, 'REPPnP')
                    ...
                    tcost = toc;
                 elseif strcmp(method_list(k).name, 'RREPPnP')
                     tic;
                     X = [XXw, XXwo];
                     x = [xxn, xxo];
                     K = [f 0 0;0 f 0; 0 0 1];
                     [R1,t1,~,robustiters] = RREPPnP(K,X,x);
                     tcost = toc;
                 else
                    try
        ...
        save planar3DresultsOutliersRansac_lastiter method_list npt pouts ;
end

save planar3DresultsOutliersRansac method_list npt pouts ;

plotPlanar3DoutliersRansac;