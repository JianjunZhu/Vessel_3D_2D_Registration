function Display3DA2DVesselDataMatching(VesData3D,K,VesData2D,varargin)
...
% 
% maxv = round(max(VesData2D.VesselPoints,[],1))+1;
% minv = round(min(VesData2D.VesselPoints,[],1))-1;
h=figure;
...
spacing = 0;
VesData2D.VesselPoints(:,1) = VesData2D.VesselPoints(:,1) + spacing;
VesData2D.VesselPoints_Sparse(:,1) = VesData2D.VesselPoints_Sparse(:,1) + spacing;
    ...
    for j = 1:counts
        if VesData2D.Graph(i,j) >= 1
            plot([VesData2D.VesselPoints(i,1),VesData2D.VesselPoints(j,1)], ...
                [VesData2D.VesselPoints(i,2),VesData2D.VesselPoints(j,2)],'Color',color1,'LineWidth',2);
        end
    end
end
    ...
    for j = 1:counts
        if VesData2D_Proj.Graph(i,j) >= 1
            plot([VesData2D_Proj.VesselPoints(i,1),VesData2D_Proj.VesselPoints(j,1)], ...
                [VesData2D_Proj.VesselPoints(i,2),VesData2D_Proj.VesselPoints(j,2)],'Color',color2,'LineWidth',2);
        end
    end
end
%% Display label
